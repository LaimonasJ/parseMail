...
dictionary = {
    'subjectLength':'numeric',
    'diffEncodedDecodedText':'numeric',
    'wordCountText':'numeric',
    'totalLength':'numeric',
...
def parseMessage(message, data):
    ...
    data['subjectLength'] = len(str(message['subject']))
    ...
    for part in message.walk():
        text = ''
        data['totalLength'] += len(part.get_payload())
        if part.get_content_type() == 'text/plain':
            ...
            text = soup.get_text()
            data['diffEncodedDecodedText'] += len(part.get_payload()) - len(part.get_payload(decode=True))
        ...
        if text != '':
            data['wordCountText'] += len(text.split())
            data['textLength'] += len(text)
            data['textWordCount'] += len(text.split())
            data['alphaChars'] += len([c for c in text if c.isalpha()])
